...
open "http://sandbox.feedly.com/v3/auth/auth?response_type=code&client_id=sandbox&client_secret=$client_secret&redirect_uri=urn:ietf:wg:oauth:2.0:oob&scope=https://cloud.feedly.com/subscriptions&provider=google"
...
curl --verbose --data "code=$code&client_id=sandbox&client_secret=$client_secret&redirect_uri=urn:ietf:wg:oauth:2.0:oob&grant_type=authorization_code" http://sandbox.feedly.com/v3/auth/token > $token_response_file
